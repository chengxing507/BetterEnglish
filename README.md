...
|-------------|-----------|-----------------|
| v1.5.2 | 2026-07 | **批量非标记题复核**：错题列表多选checkbox+全选/取消+批量AI复核，状态持久化；**全局日志系统**：答题/API/配置/系统事件自动记录，管理面板筛选/查看/清空/导出/导入恢复；**加时配置扩展**：全局总次数/按比例分配两种模式；**修复多项Bug**：加时按钮不可见、`timeExtendUsed`未重置、标点处理不完整(`–``―`等)、`refreshResultStats`缺加时统计、`maxAttempts`/`disputeRatio`保存失败、监督面板未做题误计正确、错题数不响应展示正误、虚拟键盘`—`改为`-`、`adminShowFeedback`硬编码`checked`移除、`normalizeText`空格合并条件修复 / **Batch unmarked question review**: multi-select checkboxes + select all/cancel + batch AI review with persistent state; **Global log system**: auto-record answers/API calls/config changes/system events, filter/view/clear/export/import restore in admin panel; **Extended time config**: global total / proportional allocation modes; **Bug fixes**: time extend button visibility, `timeExtendUsed` not reset per question, incomplete punctuation handling(`–``―`etc), missing extend time stats in `refreshResultStats`, `maxAttempts`/`disputeRatio` save failure, supervisor panel miscounts unanswered as correct, wrong count not responding to show-feedback toggle, virtual keyboard `—`→`-`, hardcoded `checked` in `adminShowFeedback` removed, `normalizeText` space-merge condition fixed |
