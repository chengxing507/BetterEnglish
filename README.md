...
│   │   ├── main/
│   │   │   ├── assets/
│   │   │   │   ├── English_v1.4.2.html      # 主界面 (WebView 加载)
│   │   │   │   ├── English_v1.4.1.html      # 上一版本
│   │   │   │   ├── oldversion/                # 历史版本存档
│   │   │   │   ├── unit/                      # 单元题库 (JSON)
│   │   │   │   ├── Seed/                      # 数据构建脚本 (Python)
│   │   │   │   ├── tool/                      # 辅助工具页面
...
- 🤖 AI API 配置
- 📊 数据管理（导入/导出/重置）
- 🔄 **忽略标点符号**（开启后判断答案时自动忽略标点、空格和大小写差异）
- 📋 **导出错题**（结果页支持导出纯中文/纯英文/中英双语，一键复制）
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v1.4.3 | 2026-07 | 正确率细分（单词/词组/句子）、修复复核正确后错题不移除、新增忽略标点符号开关、错题导出、历史记录回到结算页 |
| v1.4.2 | 2026-07 | 答题光标支持、AI配置一键导入、格式转换提示词可编辑、JSON句子自动拆分、文件上传修复 |
| v1.4.1 | 2026-06 | 优化 UI 与倒计时体验 |
